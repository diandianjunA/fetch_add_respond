//
// Created by 熊嘉晟 on 2024/6/24.
//

#include "rdma_fetch_add_respond.h"
...
doca_error_t rdma_fetch_add_responder_export_and_connect(struct rdma_resources *resources)
{
    uint64_t psrc = 20;
    doca_error_t result;
    ...
    memcpy(resources->mmap_memrange, &psrc, sizeof(uint64_t));
...
void rdma_fetch_add_responder_state_change_callback(const union doca_data user_data,
                                               struct doca_ctx *ctx,
                                               ...
                                               enum doca_ctx_states next_state)
{
    ...
    switch (next_state) {
        case DOCA_CTX_STATE_STARTING:
            ...
            result = rdma_fetch_add_responder_export_and_connect(resources);
            if (result != DOCA_SUCCESS) {
        ...
        case DOCA_CTX_STATE_RUNNING:
            ...
            DOCA_LOG_INFO("Requester has fetch_add: \"%lu\"", *((uint64_t*)buffer));
...
doca_error_t rdma_fetch_add_responder(struct rdma_config *cfg)
{
    struct rdma_resources resources = {0};
    union doca_data ctx_user_data = {0};
    const uint32_t mmap_permissions = DOCA_ACCESS_FLAG_LOCAL_READ_WRITE | DOCA_ACCESS_FLAG_RDMA_READ | DOCA_ACCESS_FLAG_RDMA_ATOMIC;
    const uint32_t rdma_permissions = DOCA_ACCESS_FLAG_RDMA_READ | DOCA_ACCESS_FLAG_RDMA_ATOMIC;
...
//    memcpy(cfg->device_name, "mlx5_1", sizeof("mlx5_1"));
    ...
    result = doca_ctx_set_state_changed_cb(resources.rdma_ctx, rdma_fetch_add_responder_state_change_callback);
    if (result != DOCA_SUCCESS) {
